...
atlas_tex = load_texture('Textures/atlas.jpg') or 'white_cube'

TILE_SCALE = (0.5, 0.5) 
WALL_OFFSET = (0, 0.5)
FLOOR_OFFSET = (0.5, 0.5)
CEILING_OFFSET = (0, 0)
DOOR_OFFSET = (0.5, 0)
...
Entity(model='plane', collider='box', scale=150, texture=atlas_tex, texture_scale=(25, 25), texture_offset=FLOOR_OFFSET, position=(75, 0, 75), shadows=True)
Entity(model='plane', collider='box', scale=150, texture=atlas_tex, texture_scale=(25, 25), texture_offset=CEILING_OFFSET, position=(75, 4.5, 75), rotation_x=180, shadows=True)
...
main_bg = Sprite(texture=atlas_tex, texture_scale=TILE_SCALE, texture_offset=WALL_OFFSET, parent=main_menu, scale=(1.8, 1), unlit=True)
...
end_bg = Sprite(texture=atlas_tex, texture_scale=TILE_SCALE, texture_offset=CEILING_OFFSET, parent=end_menu, scale=(1.8, 1), unlit=True)
    ...
    with open(level_path) as f:
        for line in f:
            ...
            if t == 'spawn':
                ...
                exit_door = Entity(model='cube', collider='box', position=(world_x, 2.25, world_z), scale=(3, 4.5, 3), texture=atlas_tex, texture_scale=TILE_SCALE, texture_offset=DOOR_OFFSET, shadows=True)
            elif t == 'key':
                ...
                Entity(model='cube', collider='box', position=(world_x, 2.25, world_z), scale=(3, 4.5, 3), texture=atlas_tex, texture_scale=(1, 1), texture_offset=WALL_OFFSET, shadows=True)
...
def load_generated_level(seed, width=21, height=21):
    ...
    exit_door = Entity(model='cube', collider='box', position=((ex + 0.5) * 3, 2.25, (ez + 0.5) * 3), scale=(3, 4.5, 3), texture=atlas_tex, texture_scale=TILE_SCALE, texture_offset=DOOR_OFFSET, shadows=True)
...
def restart_game():
    ...
    if exit_door:
        exit_door.texture_offset = DOOR_OFFSET
...
load_generated_level(seed=12345)